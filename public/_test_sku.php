...
// NEW: enable schema fragment dumps without affecting listing submission
$dumpSchema = isset($_GET['dumpSchema']) ? (int)$_GET['dumpSchema'] : 0;

echo "<pre>";
echo "=== SCRIPT VERSION: test_sku.php v7 (schema fragment dumps for stones+ring) ===\n\n";
...
function jsonPointerGet(array $doc, string $pointer)
{
    ...
    if (str_starts_with($pointer, '#')) $pointer = substr($pointer, 1);
    if ($pointer === '') return $doc;
    if (!str_starts_with($pointer, '/')) return null;
...
final class RefStore
{
    ...
    {
        if (isset($this->mem[$url])) return $this->mem[$url];
        ...
        if (!is_array($doc)) throw new RuntimeException("Remote \$ref JSON ist ungültig: {$url}");
...
function derefNode($node, array $root, RefStore $refs, int $depth = 0)
{
    ...
    if (is_array($node) && isset($node['$ref']) && is_string($node['$ref'])) {
        ...
        if (str_starts_with($ref, '#')) {
            ...
            if ($target !== null) return derefNode($target, $root, $refs, $depth + 1);
            return $node;
        }
        ...
        if (preg_match('~^https?://~i', $ref)) {
            ...
            if ($target !== null) return derefNode($target, $doc, $refs, $depth + 1);
            return $node;
        }
    }
...
function findPropertyDefinition(array $schema, RefStore $refs, string $wantedKey): ?array
{
    ...
    while ($queue) {
        ...
        foreach ($node as $k => $v) {
            ...
            if ($k === 'allOf') {
                foreach ($v as $i => $vv) {
                    if (is_array($vv)) $queue[] = ['node' => $vv, 'path' => $path . ".allOf[{$i}]", 'root' => $root];
                }
                continue;
            }
...
function dumpSchemaFragment(string $label, array $hit): void
{
    ...
    // If it has items->properties, dump each property's enum if present
    ...
    // Also: if it has direct properties (non-items), show enum hints there too
...
function extractIssuesVerbose($response): array
{
    ...
    if (!$payload || !is_object($payload) || !method_exists($payload, 'getIssues')) return [];
...
function printIssuesVerbose(array $issues, string $label): void
{
    ...
    foreach ($issues as $i) {
        ...
        echo "- [{$sev}] {$code}: {$msg}";
        if (is_array($attrNames) && $attrNames) echo " (attributeNames: " . implode(', ', array_map('strval', $attrNames)) . ")";
        if (is_string($attrPath) && $attrPath !== '') echo " (attributePath: {$attrPath})";
        echo "\n";
    }
    echo "\n";
}
...
{
    if (preg_match('/„([^“]+)“/u', $msg, $m)) return trim((string)$m[1]);
    return null;
}
...
function parse_stones(string $raw): ?array
{
    ...
    if (!preg_match('/^\s*(\d+)\s*x\s*([0-9]*\.?[0-9]+)\s*$/i', $raw, $m)) return null;
...
function pickSuggestedAsin(array $materials): string
{
    ...
    if (in_array('carbon', $m, true)) return $asinCarbon;
    if (in_array('holz', $m, true)) return $asinWood;
    if (in_array('titan', $m, true)) return $asinTitan;
...
// NEW: targeted schema fragment dumps (stones + ring)
$hitStones = findPropertyDefinition($schema, $refs, 'stones');
if ($hitStones) dumpSchemaFragment('stones', $hitStones);
else echo "SCHEMA: stones not found\n\n";

$hitRing = findPropertyDefinition($schema, $refs, 'ring');
if ($hitRing) dumpSchemaFragment('ring', $hitRing);
else echo "SCHEMA: ring not found\n\n";
...
$baseId = is_array($normalized) && ($normalized['oo_id'] ?? '') !== '' ? (string)$normalized['oo_id'] : $model;
...
$payload = [
    ...
    'attributes' => [
        'item_name' => [[
            'value' => $text['title'] . " Größe {$ringSize}",
        ...
        'product_description' => [[
            'value' => "Beschreibung für Model E008", //$text['description'],
            'marketplace_id' => $marketplaceId,
            'language_tag' => 'de_DE',
        ]],
        'bullet_point' => array_map(static function ($b) use ($marketplaceId) {
            return [
                'value' => (string)$b,
                'marketplace_id' => $marketplaceId,
                'language_tag' => 'de_DE',
        ...
        'purchasable_offer' => [[
            ...
            'our_price' => [[
                'schedule' => [[
                    'value_with_tax' => (float)$text['price'],
                ]],
            ]],
        ]],
...
// ---- REQUIRED RING ATTRIBUTES (real keys from your issues) ----
$materials = is_array($normalized) ? ($normalized['materials'] ?? []) : [];
$suggestedAsin = is_array($normalized) ? ($normalized['suggested_asin'] ?? pickSuggestedAsin($materials)) : pickSuggestedAsin($materials);

$payload['attributes']['supplier_declared_dg_hz_regulation'] = [[
    'value' => 'not_applicable',
    'marketplace_id' => $marketplaceId,
]];

$payload['attributes']['jewelry_material_categorization'] = [[
    'value' => 'fashion',
    'marketplace_id' => $marketplaceId,
]];

$payload['attributes']['department'] = [[
    'value' => 'unisex-adult',
    'marketplace_id' => $marketplaceId,
]];
...
if (is_array($normalized) && ($normalized['ean'] ?? '') !== '') {
    $payload['attributes']['externally_assigned_product_identifier'] = [[
        'value' => (string)$normalized['ean'],
        'marketplace_id' => $marketplaceId,
    ]];
}

$payload['attributes']['gem_type'] = [[
    'value' => 'Zirkonia',
    'marketplace_id' => $marketplaceId,
]];

// stones (required) - SCHEMA: array of objects
// items.required: creation_method, id, treatment_method, type
$stoneCount = (int)(is_array($normalized) && is_array($normalized['stones'] ?? null) ? ($normalized['stones']['count'] ?? 0) : 0);
if ($stoneCount < 1) {
    $stoneCount = 1;
}

$payload['attributes']['stones'] = [[
    'marketplace_id' => $marketplaceId,
    'id' => 1,
    'type' => [
        'language_tag' => 'en_US',
...
// ring (required) - SCHEMA: array of objects
// items.properties: marketplace_id, size, sizing_lower_range, sizing_upper_range
// size has minItems: 1 => must be present.
// IMPORTANT: Also NO "value" wrapper here.
$payload['attributes']['ring'] = [[
    'marketplace_id' => $marketplaceId,
    'size' => [[
...
$payload['attributes']['country_of_origin'] = [[
    'value' => 'CN',
    'marketplace_id' => $marketplaceId,
]];

if (is_array($normalized) && !empty($normalized['materials_display'])) {
    $payload['attributes']['material'] = [[
        'value' => (string)$normalized['materials_display'],
        'marketplace_id' => $marketplaceId,
    ]];
} elseif (is_array($normalized) && !empty($normalized['materials'])) {
    $payload['attributes']['material'] = [[
        'value' => (string)$normalized['materials'][0],
        'marketplace_id' => $marketplaceId,
    ]];
}

$payload['attributes']['is_resizable'] = [[
    'value' => false,
    'marketplace_id' => $marketplaceId,
]];

$payload['attributes']['merchant_suggested_asin'] = [[
    'value' => (string)$suggestedAsin,
    'marketplace_id' => $marketplaceId,
]];

// recommended_browse_nodes (keep scalar, because your validator rejects array form)
$payload['attributes']['recommended_browse_nodes'] = [[
    'marketplace_id' => $marketplaceId,
    'value' => 11961464031,
]];
...
try {
    $resp = $amazon->createOrUpdateListing([
        'sellerSku' => $sellerSku,
        ...$payload,
    ]);
    ...
    echo "=============================\n\n";
    foreach ($missingLabels as $l) echo "• {$l}\n";
    echo "\n";
